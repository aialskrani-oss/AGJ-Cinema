...
export default function Watch() {
  ...
  // Parse route: /watch/movie/:id  OR  /watch/tv/:id/:season/:episode
  const isTV    = location.startsWith("/watch/tv/");
  const parts   = location.split("/").filter(Boolean); // ["watch","tv","id","s","e"]
  const id      = parts[2] ?? "";
  ...
  useEffect(() => { resetHide(); return () => { if (hideTimer.current) clearTimeout(hideTimer.current); }; }, []); // eslint-disable-line
