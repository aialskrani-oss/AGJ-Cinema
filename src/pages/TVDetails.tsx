...
export default function TVDetails() {
  ...
  const similarShows: Movie[] = similar?.map(tvToMovie) ?? [];
  ...
  return (
    <div className="min-h-screen bg-[#141414] animate-fadeIn pb-24 md:pb-0">
        ...
        <div className="flex gap-6 md:gap-8">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm mb-4">
              ...
              {show.number_of_seasons && <span className="text-white/60">{show.number_of_seasons} Season{show.number_of_seasons > 1 ? "s" : ""}</span>}
              {show.number_of_episodes && <span className="text-white/60">{show.number_of_episodes} Episodes</span>}
              {show.episode_run_time?.[0] && <div className="flex items-center gap-1 text-white/60"><Clock className="w-4 h-4" /><span>~{show.episode_run_time[0]}m / ep</span></div>}
              {year && <span className="text-white/60">{year}</span>}
              {show.status && <span className="bg-cyan-400/20 text-cyan-400 text-xs px-2 py-0.5 rounded-full border border-cyan-400/30">{show.status}</span>}
            </div>

            {show.genres && show.genres.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-4">
                {show.genres.map(g => <span key={g.id} className="bg-white/10 text-white/80 text-xs px-3 py-1 rounded-full border border-white/10">{g.name}</span>)}
              </div>
            )}
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="relative">
              <select value={selectedSeason} onChange={e => setSelectedSeason(Number(e.target.value))}
                ...
                {mainSeasons.map(s => <option key={s.season_number} value={s.season_number}>Season {s.season_number}</option>)}
              </select>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {seasonData.episodes.map(ep => (
                <div key={ep.id} className="bg-[#181818] rounded-xl overflow-hidden border border-white/5 hover:border-cyan-400/30 transition-all duration-200 group">
                  <div className="relative aspect-video bg-gray-800 cursor-pointer" onClick={() => watchEpisode(ep.season_number, ep.episode_number)}>
                    {ep.still_path
                    ...
                    <div className="absolute top-2 left-2 bg-black/70 text-white/80 text-xs px-2 py-0.5 rounded-md font-medium">E{ep.episode_number}</div>
                    {ep.vote_average > 0 && (
                  ...
                  <div className="p-3">
                    ...
                    <button onClick={() => watchEpisode(ep.season_number, ep.episode_number)}
      ...
      {similarShows.length > 0 && (
        <div className="mt-10">
          <MovieRow title="More Like This" movies={similarShows} onSelect={(m: Movie) => navigate(`/tv/${m.id}`)} onPlay={(m: Movie) => navigate(`/tv/${m.id}`)} mediaType="tv" />
        </div>
      )}
    </div>
  );
